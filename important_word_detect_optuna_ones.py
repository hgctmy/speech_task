...
class ImportantWordDataset(Dataset):
    def __init__(self, preprocessed_dir, filter_ones=False):
        ...
        for file_name in os.listdir(preprocessed_dir):
            if file_name.endswith('.pt'):
                ...
                if filter_ones:
                    # ラベルに1が含まれるデータのみをフィルタリング
                    filtered_data = []
                    for utterance in batch_data:
                        # ラベルに1が含まれるかチェック
                        if torch.any(utterance['labels'] == 1):
                            filtered_data.append(utterance)
...
def objective(trial):
    ...
    d_model_base = trial.suggest_int('d_model_base', 16, 128, step=16)
    ...
    # データセットの準備 - ラベルに1が含まれるデータのみ
    full_dataset = ImportantWordDataset('preprocessed_data_pooling', filter_ones=True)
...
def train_model_with_best_params(best_params):
    ...
    model = ImportantWordClassifier(
        d_model=best_params['d_model_base']*best_params['nhead'],
    ...
    patience_counter = 0
    start_epoch = 0
    checkpoint_path = 'best_model_tuned_ones_v2.pth'
...
'''
[I 2025-03-07 17:55:12,213] Trial 199 pruned. 
Number of finished trials: 200
Best trial:
  Value: 0.3446812331676483
  Params: 
    nhead: 8
    d_model_base: 128
    num_layers: 6
    lr: 6.494648793869072e-05
    weight_decay: 0.0033994605387908015
    focal_alpha: 0.5313620731353329
    focal_gamma: 1.633313687217896
    batch_size: 8
'''