...
def custom_bce_with_logits_loss(logits, target, weight=None, ignore_index=-100):
    ...
    if weight is not None:
        weight = weight.to(logits.device) # weightもlogitsと同じdeviceに移動
...
class SpeechImportanceModel(nn.Module):
    ...
    def forward(self, input_values, attention_mask=None, labels=None): # weightsを削除
        ...
        if labels is not None:
            # ここでweightを適用
            if hasattr(self, 'loss_weight'): # weightが設定されているか確認
